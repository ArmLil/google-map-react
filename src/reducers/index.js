    ...
    case 'UPDATE_PLACE':
      return state.map(elem => {
        if(elem == action.place) {
          const newObj = Object.assign({},elem,action.newPlace,{id:action.newPlace.place.place_id,location:action.newPlace.place.geometry.location})
          return newObj
        }
        return elem
      })
...
const getCenter = (state = {lat:55.755826,lng: 37.61}, action) => {
  switch (action.type) {
    case 'SET_CENTER':
